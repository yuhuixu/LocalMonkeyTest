...
def get_men(pkg_name, devices):
   ...
   107783 kB: com.zzl.falcon.internal (pid 4682 / activities)
    18195 kB: com.zzl.falcon.internal:channel (pid 4764)
    10578 kB: com.zzl.falcon.internal:leakcanary (pid 4927 / activities)
              107783 kB: com.zzl.falcon.internal (pid 4682 / activities)
               18195 kB: com.zzl.falcon.internal:channel (pid 4764)
               10578 kB: com.zzl.falcon.internal:leakcanary (pid 4927 / activities)
    :param pkg_name:
    ...
    if pid is not None:
        ...
        for item in _flow:
            ...
            if type == "gprs" and item.split()[0].decode() == "rmnet0:":  # gprs
                ...
                print(upflow)
                break
            if type == "virtual_machine" and item.split()[0].decode() == "eth1:":  # eth0
                print("-----flow---------")
                upflow = int(item.split()[1].decode())
                downflow = int(item.split()[9].decode())
                print(upflow)
...
def cpu_rate(pid, cpukel, devices):
    ...
    # print("cpu_rate:totalCpuTime3="+str(totalCpuTime3))
    # print("cpu_rate:processCpuTime3="+str(processCpuTime3))
    #
    ...
    cpu=float(cpu_rate[0].split('%')[0]) / 100
